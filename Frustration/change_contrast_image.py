...
parser.add_argument('--diffexp', action='store_true', help='whether matching for each experiment or not')
parser.add_argument('--excludewhite', action='store_true', help='whether excluding white or not')
parser.add_argument('--backgroundcolor', default=0, type=int, help='background color')
parser.add_argument('--mean', default=80, type=float, help='custom mean for custom zscore')
parser.add_argument('--std', default=80, type=float, help='custom mean for custom zscore')
...
def main():
    ...
    if mode == '2shoulder':
        target_path = f'{spath[:-1]}_{mode}_{method}_{space}_diffexp{diffexp}_excludewhite{excludewhite}_background{backgroundcolor}'
        target_hist_path = f'/mnt/cube/projects/xiaojing/{space}value_shoulderpain_histogram.npz'
    if mode == 'w2b':
        target_path = f'{spath[:-1]}_{mode}_{method}_{space}_diffexp{diffexp}_excludewhite{excludewhite}_background{backgroundcolor}'
    elif mode == 'custom':
        target_path = f'{spath[:-1]}_{mode}_{method}_{space}_diffexp{diffexp}_excludewhite{excludewhite}_mean{mean}std{std}_background{backgroundcolor}'
    ...
    for (root,dirs,files) in os.walk(spath, topdown=True): 
        ...
        for f in tqdm(os.listdir(root)):
            ...
            if space == 'hsv':
                matched_img = cv2.cvtColor(hsv_img, cv2.COLOR_HSV2RGB)
            elif space == 'hsi':
